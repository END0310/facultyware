...
const approvePermohonan = async (req, res) => {
    try {
        const id = req.params.id;
        await db.query("UPDATE equipment_procurements SET status = 'approved' WHERE id = ?", [id]);
...
const rejectPermohonan = async (req, res) => {
    try {
        const id = req.params.id;
        await db.query("UPDATE equipment_procurements SET status = 'rejected' WHERE id = ?", [id]);
        res.redirect('/wakildekan/permohonan');
    } catch (error) {
        console.error(error);
        res.status(500).send('Server error');
