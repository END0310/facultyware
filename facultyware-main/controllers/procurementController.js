...
const listRequests = async (req, res, next) => {
  try {
    ...
    `);
    res.render('procurements/requests/index', { title: 'Daftar Usulan Pengadaan', requests });
  } catch (err) {
    next(err);
  }
};
...
const detailRequest = async (req, res, next) => {
  try {
    ...
    res.render('procurements/requests/detail', { title: 'Detail Usulan', request: rows[0], requestStatuses });
  } catch (err) {
    next(err);
  }
};
...
const showCreateProcurement = async (req, res, next) => {
  try {
    ...
    res.render('procurements/create', { title: 'Buat Permohonan Pengadaan', sourceRequest });
  } catch (err) {
    next(err);
  }
};
...
const listProcurements = async (req, res, next) => {
  try {
    ...
    `);
    res.render('procurements/index', { title: 'Daftar Permohonan Pengadaan', procurements, rupiah });
  } catch (err) {
    next(err);
  }
};
...
const detailProcurement = async (req, res, next) => {
  try {
    ...
    res.render('procurements/detail', { title: 'Detail Permohonan', procurement, items, rupiah });
  } catch (err) {
    next(err);
  }
};
...
const decideProcurement = async (req, res, next) => {
  ...
  try {
    // Placeholder untuk integrasi Wakil Dekan. Tabel procurement saat ini tidak punya kolom approved_by/approved_at.
    const [result] = await db.query(`
      UPDATE equipment_procurements
    ...
    flash(req, 'success', `Permohonan berhasil di-${decision}.`);
    res.redirect(`/procurements/${req.params.id}`);
  } catch (err) {
    next(err);
  }
...
const showAddAsset = async (req, res, next) => {
  try {
    ...
    res.render('procurements/add-asset', {
...
const addAssetFromProcurement = async (req, res, next) => {
  ...
  try {
    ...
    if (!assetCode || !name || !isValidDateInput(acquisitionDate) || !isFiniteNonNegativeNumber(acquisitionCost)) {
      await conn.rollback();
      return res.status(422).render('procurements/add-asset', {
        title: 'Tambah Barang Hasil Pengadaan',
        procurement,
        items,
    ...
    if (items.length === 1 && totalQuantity <= 1) {
      await conn.query(`UPDATE equipment_procurements SET status = 'completed', updated_at = NOW() WHERE id = ?`, [req.params.id]);
    }
    await conn.commit();
    flash(
      req,
      ...
      items.length === 1 && totalQuantity <= 1
        ? 'Barang berhasil ditambahkan ke sistem aset dan permohonan menjadi completed.'
        : 'Barang berhasil ditambahkan ke sistem aset. Status permohonan tetap approved agar pencatatan item lain dapat dilanjutkan.'
    );
    res.redirect(`/procurements/${req.params.id}`);
  } catch (err) {
    await conn.rollback();
    if (err.code === 'ER_DUP_ENTRY') {
      ...
      const items = await getProcurementItems(req.params.id);
      const selectedItem = items.find((item) => Number(item.id) === selectedItemId) || items[0] || null;
      return res.status(409).render('procurements/add-asset', {
        title: 'Tambah Barang Hasil Pengadaan',
        procurement,
        items,
        selectedItemId: selectedItem ? Number(selectedItem.id) : null,
...
const report = async (req, res, next) => {
  try {
    ...
    res.render('procurements/report', {
